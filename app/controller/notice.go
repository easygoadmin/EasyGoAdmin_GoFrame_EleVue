...
func (c *noticeCtl) Add(r *ghttp.Request) {
	...
	// 调用添加方法
	id, err := service.Notice.Add(req, utils.Uid(r))
	if err != nil || id == 0 {
		r.Response.WriteJsonExit(common.JsonResult{
			Code: -1,
			Msg:  err.Error(),
...
func (c *noticeCtl) Update(r *ghttp.Request) {
	...
	// 调用更新方法
	rows, err := service.Notice.Update(req, utils.Uid(r))
	if err != nil || rows == 0 {
		r.Response.WriteJsonExit(common.JsonResult{
			Code: -1,
			Msg:  err.Error(),
...
func (c *noticeCtl) Delete(r *ghttp.Request) {
	...
	// 调用删除方法
	rows, err := service.Notice.Delete(req.Ids)
	if err != nil || rows == 0 {
		r.Response.WriteJsonExit(common.JsonResult{
			Code: -1,
			Msg:  err.Error(),
...
func (c *noticeCtl) Status(r *ghttp.Request) {
	...
	result, err := service.Notice.Status(req, utils.Uid(r))
	if err != nil || result == 0 {
		r.Response.WriteJsonExit(common.JsonResult{
			Code: -1,
			Msg:  err.Error(),
